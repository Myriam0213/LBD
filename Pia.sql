...
inner join CentroDeServicio on ContratoAdopcion.clave_c1 = CentroDeServicio_c;

select * from ContratoAdopcion
inner join Adoptante on ContratoAdopcion.clave_c2 = Adoptante_c GROUP BY ContratoAdopcion
having (do);

select * from ContratoAdopcion
inner join Rescatista on ContratoAdopcion.clave_c3 = Rescatista_c GROUP BY ContratoAdopcion;

select * from ContratoAdopcion
inner join IdentidadMascota on ContratoAdopcion.clave_c4 = IdentidadMascota_c GROUP BY ContratoAdopcion;

select * from ContratoAdopcion
inner join Domicilio on ContratoAdopcion.clave_c5 = Domicilio_d  GROUP BY Domicilio_d;  
...
HAVING COUNT(Id) > 10;
 ...
 SELECT COUNT(Id), TipoMascota_IM
  FROM IdentidadMascota
 GROUP BY Gatos
HAVING COUNT(Id) <= 2;

 #---------------------------------------------------------------------------------------------------------------------------#

SELECT COUNT(Id), FechaNacimiento_A
  FROM Adoptante
 GROUP BY FechaNacimiento_A
HAVING COUNT(Id) < 18;

 #---------------------------------------------------------------------------------------------------------------------------#

SELECT * FROM ContratoAdopcion WHERE COSTO_CA > 200.00
(SELECT Vacunacion FROM IdentidadMascota WHERE Descuento >= 0 .25);

#-----------------------------------------------------------------------------------------------------------------------------#

SELECT Apellido_A, Nombre_A, Edad, CorreoElectronico, FechaNacimiento
FROM Adoptante AS Registro
WHERE Edad = (FechaNacimiento - year (2019));
if (Edad =<18) (
	'Aceptado para adoptar'
)else(
	'No aceptado para adoptar'
);
...
SELECT COUNT(Adoptante_id)
FROM Adoptante;

#----------------------------------------------------------------------------------------------------------------------------#

SELECT AVG(Costo_CA)
FROM ContratoAdopcion;

#-----------------------------------------------------------------------------------------------------------------------------#

if Costo_CA > 200 THEN 
   
   True Validation in ContratoAdopcion;
END IF;
